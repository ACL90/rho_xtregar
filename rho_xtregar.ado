		...
		//the general case
			if ("`approx'"=="") & ("`approx_unbalanced'"=="") & ("`approx_balanced'"=="") {
				...
				**********
				...
				//the optimisation
				mata: rc = mm_root(r=.,&funcfinal(), 0, 1, 0, 1000,myrhoD /*+0.001*/,mynum,myNbrindvN,A,vecN2)
				mata: mm_root(r=.,&funcfinal(), 0, 1, 0, 1000,myrhoD /*+0.001*/,mynum,myNbrindvN,A,vecN2)
				mata: r
				mata: st_numscalar("final",r)
				mata: st_numscalar("rc",rc)
				restore
			}
			//
			}
			//
			if rc!=0 {
				di as error "***rho_BFN is out of the range [0-1]: result is not reliable***"
				di as error "***Such errors are currently under investigation***"
				exit
			}
		...
		//with option approx 
			if ("`approx'"!="") | ("`approx_unbalanced'"!="") | ("`approx_balanced'"!="") {			
				...
				if inlist("`balancing'","weakly balanced","strongly balanced") | "`approx_balanced'"!="" {
					scalar final = `the_rhod'/(1-2/(`thegrandT'+1))
				}
				//	
